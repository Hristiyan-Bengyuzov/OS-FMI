...
awk 'END { print NF * NR }' ~/emp.data
awk '{ fields += NF } END { print fields }' ~/emp.data

# броя редове, в които се среща низът Beth
awk '/Beth/ { matches++ } END { print matches }' ~/emp.data

# най-голямото трето поле и редът, който го съдържа
awk 'NR == 1 || $3 > max { max = $3; row = $0 } END { printf "Max: %s\nRow: %s\n", max, row }' ~/emp.data
...
awk 'BEGIN { sum = 0 } { sum += $2 + $ 3} END { printf "Sum: %.2f\n", sum }' ~/emp.data